...
/**
 * Command line option parser. Used by command line applications such as
 * Enhancer and SchemaTool to process command line arguments.
 */
public class AS2CommandLine {
	/** Appended options */
	protected Map<String, Option> options = new HashMap<String, Option>();

	/** Appended options */
	protected Map<String, Option> valueOptions = new HashMap<String, Option>();
	...
	protected static class Option {
		 ...
		 * 
		 * @param shortName
		 *            Short name option(exclude "-")
		 * @param longName
		 *            Long name option(exclude "--")
		 * @param desc
		 *            option description
		 */
	...
	protected static class WithArgOption extends Option {
		 ...
		 *            argment name
		 */
		public WithArgOption(String shortName, String longName, String desc,
				String name) {
			super(shortName, longName, desc);
			this.name = name;
		}
	 ...
	 **/
	public AS2CommandLine(boolean displaysDash) {
	 ...
	 * 
	 * @param shortName
	 *            Short name option eg "d"
	 * @param longName
	 *            Long name option eg "directory"
	 * @param argName
	 *            Argment name. No argment option if this param is null.
	 * @param desc
	 *            Desription this option.
	 */
	public void addOption(String shortName, String longName, String argName,
			String desc) {
		Option option = null;
		if (AS2String.isEmpty(shortName) && AS2String.isEmpty(longName)) {
			throw new IllegalArgumentException("require shortName or longName");
		}

		if (AS2String.notEmpty(argName)) {
		...
		if (AS2String.notEmpty(shortName)) {
			options.put("-" + shortName, option);
			valueOptions.put(shortName, option);
		}

		if (AS2String.notEmpty(longName)) {
	...
	public void parse(String args[]) {
		List<String> defaultArg = new ArrayList<String>();
		if ((args == null || (args.length == 0))) {
			return;
		}
		int i = 0;
		while (i < args.length) {
			if (AS2String.isEmpty(args[i])) {
				// do nothing
				...
				if (options.containsKey(args[i])) {
					...
					if (option instanceof NoArgOption) {
						...
						if (args.length - 1 == i) {
							throw new RuntimeException("option " + args[i]
									+ " needs an argument");
						}
	...
	public boolean hasOption(String name) {
		...
		return AS2String.notEmpty(((WithArgOption) option).option);
	}
	...
	public String toString() {
		...
		for (int i = 0; i < optionList.size(); i++) {
			int j = 0;
			Option o = optionList.get(i);
			if (AS2String.notEmpty(o.shortName)) {
			...
			j = 0;
			if (AS2String.notEmpty(o.longName)) {
				sb.append("--");
			...
			if (AS2String.notEmpty(o.description)) {
				int basePos;
				...
				for (j = 0; j < o.description.length(); j++) {
					...
					if (pos >= maxLength) {
						if (j < o.description.length() - 1
								&& o.description.charAt(j + 1) != ' ') {
							// just do not break work in the middle, and wrap
							// word to next line
